...
public class WxMpServiceInstance {
	...
	public WxMpServiceInstance() {
		try {
			System.out.println("---WxMpServiceInstance");
			...
			System.out.println("---getAccessToken" + wxMpConfigStorage.getAccessToken());
			System.out.println("---getSecret" + wxMpConfigStorage.getSecret());
	...
	private void buildMenu() {
		...
		takeout.setUrl("https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxf7e99c474fcc59f7&redirect_uri=http%3A%2F%2Fbingkuaix3.imwork.net%2Fx5%2FUI2%2FWxInterface%2Ftest.w&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect");
	...
	public WxMediaUploadResult upload() throws WxErrorException {
		File f = new File("E:\\WeX5_V3.2.1\\apache-tomcat\\webapps\\a\\1.jpg");
		WxMediaUploadResult res = wxMpService.mediaUpload("image", f);
		res.getType();
		res.getCreatedAt();
		res.getMediaId();
		res.getThumbMediaId();
	...
	public void download(String mediaid) throws WxErrorException {
		// TODO 自动生成的方法存根
		File f = wxMpService.mediaDownload(mediaid);
		String s="E:/WeX5_V3.2.1/apache-tomcat/webapps/img/";
		File newpath = new File(s);
		if (!newpath.exists()) {
			newpath.mkdirs();
		}
		File newfile=new File(s+mediaid+".jpg");
		f.renameTo(newfile);
	}
